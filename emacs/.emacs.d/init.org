...
#+BEGIN_SRC emacs-lisp
  ...
  (global-set-key (kbd "M-<f5>") 'auto-revert-mode)
#+END_SRC
...
one command and remap it to =C-a.= Calls ~back-to-indentation~, then if already at
end of indentation, calls ~move-beginning-of-line.~
